...
available_scripts=()
if [ -f "ascii-enc.py" ]; then
    available_scripts+=("ascii-enc.py")
fi
...
if [ ${#available_scripts[@]} -eq 0 ]; then
    echo -e "${RED}Error code 001:${RESET} ascii-enc.py was not found in the current directory."
    exit 1
fi
...
echo "Installing '$source_file' to /usr/bin/ascii-enc..."

# Copy the chosen file to /usr/bin
sudo cp "$source_file" /usr/bin/ascii-enc
...
if [ -f "/usr/bin/cpuinfo" ]; then
    echo "Success! '$source_file' is now installed as 'ascii-enc'."
    echo "You can run it from any directory by typing 'ascii-enc'."
else
